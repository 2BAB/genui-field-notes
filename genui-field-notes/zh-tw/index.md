...
聊天訊息裡出現的一張精緻卡片、模型生成一段 JSON 交給用戶端呈現、頁面根據使用者輸入重新組織版面或調整主題，如今都可能被歸到同一個詞底下：Generative User Interface, aka GenUI。

實際上，對工程師來說，把範圍收斂到技術層面會更容易理解：裝置端 UI 如何根據意圖、狀態、工具、資料和回饋，動態生成、調整或組裝介面。從這個角度看，GenUI 涵蓋大家日常使用的 Web 頁面與各類 App，也包括正在嘗試即時生成的遊戲等實驗。
...
4. 引用內容主要來自 OpenUI、A2UI、Thesys C1、Vercel AI SDK 等相關官方文件、GitHub Repo、Demo、技術文章和產品實驗。
...
鼓勵大家多把 AI 用在前期的發現與探索、Demo 實驗和技術討論，而不是用來產出小冊本身的核心想法。想參與小冊編輯，請前往 GenUI Field Notes 的 [GitHub Repo](https://github.com/2BAB/genui-field-notes)。其他相關問題，歡迎[聯絡我](https://2bab.me/about)。
